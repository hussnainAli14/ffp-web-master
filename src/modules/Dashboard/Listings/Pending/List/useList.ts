...
import { approveProduct, getListProductByStatus, removeProduct } from '@ffp-web/lib/product/data';
...
const useList = (): UseList => {
  ...
  const handleApprove = (row: ProductListProps) => {
    setSelectedProduct(row);
  ...
  const _approveProduct = async () => {
    try {
      ...
      if (selectedProduct?.productId) {
        ...
        if (data) {
          toast.success(`"${selectedProduct?.productName}" succesfully approved`);
          setSelectedProduct(undefined);
        } else {
          toast.error(message);
        }
