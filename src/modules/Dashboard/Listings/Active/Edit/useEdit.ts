...
import { addProduct, editProduct, getProductDetail, removeProduct } from '@ffp-web/lib/product/data';
...
const useEdit = (): UseEdit => {
  ...
  const onSubmit = (data: AddForm) => {
    ...
    _editActiveProduct(payload);
  };

  const onSavaPending = () => {
    const data = watchedStates;
    const payload: ProductDetailProps = {
  ...
  const _editActiveProduct = async (payload: ProductDetailProps) => {
    try {
  ...
  const _addPendingProduct = async (payload: ProductDetailProps) => {
    try {
      setIsLoadingSubmit(true);
      const { data, message } = await addProduct(payload, getToken());
      if (data) {
        toast.success(`"${payload.name}" successfully added to pending listings`);
