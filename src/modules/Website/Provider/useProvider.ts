...
import { ProviderForm, UseProvider } from './Provider.types';
...
const useProvider = (): UseProvider => {
  ...
  const _addProvider = async (payload: AddUserProps) => {
    try {
      ...
      if (data) {
        toast.success(
          'Thank you for submitting, our team will contact you within 24 hours for the next step',
          { delay: 10000 }
        );
        router.push('/become-a-provider/submit');
      } else {
        toast.error(message);
      }
    } catch (err) {
